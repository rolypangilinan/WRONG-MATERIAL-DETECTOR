...
# EXCEL_DIR = r"\\192.168.2.19\ai_team\INDIVIDUAL FOLDER\June-San\p2LTG\p2LTG_TransferData\WRONG MATERIAL DETECTOR\8. JOB ORDER MATERIAL LIST" # ORIGINAL PATH
...
COLUMN_MAPPING_P2 = {
    'Process 2 M4x40 Screw': 9,
    'Process 2 Rod Blk': 11,
    'Process 2 Df Blk': 13,   #OK
...
ML_TARGET_COLUMNS_P2 = ['Process 2 M4x40 Screw', 'Process 2 Rod Blk', 'Process 2 Df Blk',
                        'Process 2 Df Ring', 'Process 2 Washer', 'Process 2 Lock Nut'] # columns 11-16 in output CSV
# Column names
...
# GUI setup
root = tk.Tk()
# === DYNAMIC TITLE: Now uses __file__ to get actual running script name ===
root.title(f"Production Monitoring System - PYTHON FILE NAME: {os.path.basename(__file__)}")
root.geometry("1100x800")
root.configure(bg="#f0f0f0")
# === 7TH: JOB ORDER DISPLAY (Top Center - Pleasing) ===
jo_frame = tk.Frame(root, bg="#f0f0f0")
jo_frame.pack(pady=8)
jo_label_title = tk.Label(jo_frame, text="CURRENT JOB ORDER:", font=("Arial", 12, "bold"), bg="#f0f0f0", fg="#04080C")
jo_label_title.pack(side="left", padx=5)
jo_display = tk.Label(jo_frame, text="N/A", font=("Arial", 14, "bold"), bg="#ffffff", fg="#2187EE", relief="sunken", borderwidth=15, width=20, anchor="center")
jo_display.pack(side="left")
# === 1ST & 2ND: BACK & NEXT BUTTONS (Top Left & Right) ===
nav_frame = tk.Frame(root, bg="#f0f0f0")
nav_frame.pack(fill="x", pady=5)
back_btn = tk.Button(nav_frame, text="BACK", font=("Arial", 10, "bold"), bg="#686663", fg="white", relief="ridge", borderwidth=8, width=10)
back_btn.pack(side="left", padx=10)
next_btn = tk.Button(nav_frame, text="NEXT", font=("Arial", 10, "bold"), bg="#686663", fg="white", relief="ridge", borderwidth=8, width=10)
...
prev_btn = tk.Button(control_frame, text="PREVIOUS", font=("Arial", 10, "bold"), bg="#E8D7EB", fg="black", width=10)
prev_btn.pack(side="right", padx=5)
# prev_btn.grid(row=0, column=0, padx=5)
refresh_btn = tk.Button(control_frame, text="REFRESH", font=("Arial", 10, "bold"), bg="#E8D7EB", fg="black", width=10)
refresh_btn.pack(side="right", padx=5)
stop_all_btn = tk.Button(control_frame, text="STOP ALL", font=("Arial", 10, "bold"), bg="#f44336", fg="white", width=10)
stop_all_btn.pack(side="right", padx=10)
# === 5TH: SYSTEM MESSAGE TEXT BOX ===
msg_frame = tk.Frame(root, bg="#f0f0f0") # "#f0f0f0" WHITE, BLACK "#000000"
...
system_text = scrolledtext.ScrolledText(msg_frame, height=6, font=("Courier", 9), state="disabled", bg="#000000", fg="#f0f0f0", relief='sunken', borderwidth=15)
...
clear_material_button = tk.Button(clear_btn_frame, text="CLEAR LOG", font=("Arial", 10, "bold"), bg="#ff4444", fg="white",
                                  width=15, relief="raised", borderwidth=5, command=clear_material_log)
...
def process_job_order():
    ...
    try:
        ...
        with open(OUTPUT_PATH, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            header = ["Job Order Number", "File Name"] + list(COLUMN_MAPPING.keys()) + list(COLUMN_MAPPING_P2.keys())
            writer.writerow(header)
            empty_row = [cleaned_job_order, ""] + [""] * len(COLUMN_MAPPING) + [""] * len(COLUMN_MAPPING_P2)
...
def update_from_log000_2(force_update=False):
    ...
    try:
        ...
        return False
# === SKLEARN VALIDATION + CORRECTION (MODIFIED TO SUPPORT PROCESS 1 & 2) ===
def validate_material_in_excel(output_rows, process_id=1):
    try:
        ...
        any_error = False
        empty_error = False
        target_columns = ML_TARGET_COLUMNS if process_id == 1 else ML_TARGET_COLUMNS_P2
...
# === WATCHDOG HANDLER (NOW HANDLES BOTH LOG000_1 AND LOG000_2) ===
...
# === MONITORING LOOP ===
def monitoring_loop():
    global animation_running, last_mtime_log1, last_mtime_log2
    observer = None
    dot_count = 0
    try:
        process_job_order()
        # Initial load for both logs
...
# === BUTTON COMMANDS ===
def refresh_all():
    global last_mtime_log1, last_mtime_log2
    for i in range(1, 7):
