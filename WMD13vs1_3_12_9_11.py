...
COLUMN_MAPPING_P2 = {
    'Process 2 M4x40 Screw': 9, # log000_2.csv
...
COLUMN_MAPPING_P3 = {
    'Process 3 Frame Gasket': 9, # log000_3.csv
...
ML_TARGET_COLUMNS_P4 = ['Process 4 Tank',
                        ...
                        'Process 4 M4x12 Screw'] # NEW: PROCESS 4
...
# GUI setup
root = tk.Tk()
# === DYNAMIC TITLE: Now uses __file__ to get actual running script name + JO DATE ===
# Initial title (will be updated after JO is read)
root.title(f"Production Monitoring System      |      PY FILE NAME: {os.path.basename(__file__)}      |     JO DATE: N/A")
...
back_btn = tk.Button(clear_btn_frame, text="BACK", font=("Arial", 10, "bold"), bg="#686663", fg="white", relief="ridge", borderwidth=8, width=10)
...
def update_title_with_jo_date():
    ...
    root.title(f"Production Monitoring System      |      PY FILE NAME: {os.path.basename(__file__)}      |     JO DATE: {jo_date_str}")
...
def update_from_log000_3(force_update=False):
    ...
    try:
        ...
        with open(LOG000_3_PATH, 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                if row and any(c.strip() for c in row):
                    for src_idx_1based in COLUMN_MAPPING_P3.values():
                        if len(row) >= src_idx_1based:
                            val = row[src_idx_1based - 1].strip()
                            if val:
                                src_values[src_idx_1based] = val
        if not src_values:
            return False
        if not os.path.exists(OUTPUT_PATH):
            return False
        ...
        for col_name, src_idx_1based in COLUMN_MAPPING_P3.items():
            header_idx = headers.index(col_name)
            value = src_values.get(src_idx_1based, "")
            data_row[header_idx] = value
            if value:
